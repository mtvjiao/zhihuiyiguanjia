...
class CalendarController extends ComController
{
    ...
    public function index()
    {
        ...
        for ($start = $begintime; $start <= $endtime; $start += 24 * 3600) {
            ...
            foreach ($dayjihua as $key => $value) {
                # code...
                $dayjihuas = M('jihuazhixing')->field('id')->where("jihuasid='{$value[id]}' and starttime <= '{$time}' and stoptime > '{$time}' and success = 0")->find();
    ...
    public function week()
    {
        ...
        for ($start = $begintime; $start <= $endtime; $start += 24 * 3600 * 7) {
            ...
            foreach ($data_id as $key => $value) {
                # code...
                $weekjihuas = M('jihuazhixing')->field()->where("jihuasid='{$value}' and starttime <= '{$time}' and stoptime > '{$time}' and success = 0")->select();
