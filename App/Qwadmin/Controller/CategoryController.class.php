...
class CategoryController extends ComController {
	...
	public function edit(){
		...
		$categorys = M('patrol')->field('id,name')->order('orderid asc')->select();
		$trees = new Tree($categorys);
		$strs = "<option value=\$id \$selected>\$spacer\$name</option>"; //生成的形式
		$categorys = $trees->get_tree(0,$strs, $currentcategory['prid']);
		
		$this->assign('categorys',$categorys);
		$this->assign('option',$option);

		$categoryp = M('project')->field('id,name')->order('orderid asc')->select();
		$treep = new Tree($categoryp);
		$strp = "<option value=\$id \$selected>\$spacer\$name</option>"; //生成的形式
		$categoryp = $treep->get_tree(0,$strp, $currentcategory['projectid']);
		
		$this->assign('categoryp',$categoryp);
	...
	public function add(){
		...
		$categorys = M('patrol')->field('id,name')->order('orderid asc')->select();
		$trees = new Tree($categorys);
		$strs = "<option value=\$id >\$spacer\$name</option>"; //生成的形式
		$categorys = $trees->get_tree(0,$strs, $pid);
