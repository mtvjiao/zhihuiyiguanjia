...
class PatrolztController extends ComController {

	public function index(){
	
		
		$category = M('patrolzt')->field('id,pid,name')->order('id asc')->select();
	...
	public function edit(){
		...
		$id = isset($_GET['id'])?intval($_GET['id']):false;
		$currentcategory = M('patrolzt')->where('id='.$id)->find();
	...
	public function update($act=null){
		...
		if($id){
			if(M('patrolzt')->data($data)->where('id='.$id)->save()){
				addlog('文章分类修改，ID：'.$id.'，名称：'.$data['name']);
				$this->success('恭喜，分类修改成功！');
				die(0);
			}
		}else{
			$id = M('patrolzt')->data($data)->add();
			if($id){
