...
gs_filemanager_languages['en'][1] = '当前地址';
gs_filemanager_languages['en'][2] = '粘贴板';
gs_filemanager_languages['en'][3] = '上传';
gs_filemanager_languages['en'][4] = '新文件';
gs_filemanager_languages['en'][5] = '新目录';
gs_filemanager_languages['en'][6] = '粘贴';
gs_filemanager_languages['en'][7] = '名字';
gs_filemanager_languages['en'][8] = '类型';
gs_filemanager_languages['en'][9] = '大小';
gs_filemanager_languages['en'][10] = '最后更新';
...
gs_filemanager_languages['en'][23] = '全选';
gs_filemanager_languages['en'][24] = '取消全选';
gs_filemanager_languages['en'][25] = '反选';
...
gs_filemanager_languages['en'][49] = '刷新';
gs_filemanager_languages['en'][50] = '菜单';
...
if (jQuery) (function(jQuery){
	...
	jQuery.extend(jQuery.fn, {
		gsFileManager: function(o) {
			...
			if( o.oidname == undefined ) o.oidname = '公共资源';
			...
			hiddenElements += '<form action="' + o.script +'" id="gsUploadForm" enctype="multipart/form-data" method="post"><input type="hidden" name="opt" value="11"><input type="hidden" name="dir" value=""><input type="hidden" name="oidname" id="oidname" value="'+o.oidname+'"/>';
			...
			jQuery('#gs_dir_content').contextMenu({
				...
				function(action, el, pos) {
				    jQuery(el).doGSAction({action: action, script: o.script, type: 'context', lg: o.language, oidname: o.oidname});
			});
			...
			jQuery('#gs_uploadbutton').click(function (e){
				...
				jQuery(this).doGSAction({action: 14, script:  o.script, type: 'file', lg: o.language, oidname: o.oidname});
			});
			
			jQuery('#gs_newfilebutton').click(function (e){
				e.stopPropagation();
				jQuery(this).doGSAction({action: 2, script: o.script, type: 'file', lg: o.language, oidname: o.oidname});		
			});
			
			jQuery('#gs_newdirbutton').click(function (e){
				e.stopPropagation();
				jQuery(this).doGSAction({action: 3, script: o.script, type: 'dir', lg: o.language, oidname: o.oidname});
			});
			
			jQuery('#gs_pastebutton').click(function (e){
				e.stopPropagation();
				jQuery(this).doGSAction({script: o.script, action: 9, lg: o.language, oidname: o.oidname});
			});
			
			jQuery('#gs_selectallbutton').click(function (e){
				e.stopPropagation();
				jQuery(this).doGSAction({action: 20, script: o.script, type: 'context', lg: o.language, oidname: o.oidname});
			});
			
			jQuery('#gs_deselectbutton').click(function (e){
				e.stopPropagation();
				jQuery(this).doGSAction({action: 21, script: o.script, type: 'context', lg: o.language, oidname: o.oidname});
			});
			
			jQuery('#gs_invertselectbutton').click(function (e){
				e.stopPropagation();
				return jQuery(this).doGSAction({action: 22, script: o.script, type: 'context', lg: o.language, oidname: o.oidname});
			});
			...
			jQuery('#gs_jcrop_form').ajaxForm({
				...
				success: function (responseText, statusText, xhr, $form) {
					...
					var imageSrc = gs_makeUrl(o.script, "opt=15&oidname=" + o.oidname + "filename=" + filenamea +"&dir=" + dira + "&time="+ new Date().getTime());
			...
			function showDirs (gsfiless) {
				...
				if (gsfiless.length > 0) {
					for (var numf in gsfiless) {
						...
						gs_cur_items[curItem.id] = curItem;
						fileshtml += "<tr><td><a href='#' class='item_menu_link_holder' rel='" + curItem.id + "'><img src='/qwadmin0411/Public/qwadmin/filemanage/images/menu_icon.png'></a></td><td><div class='directory directory_info gsItem' id='gs_div_holder_" + curItem.id + "' rel=\'" + curItem.id + "\'><a href='javascript:void(0)' ondblclick=\"jQuery('#"+curItem.id+"').trigger('click'); return false\">" + curItem.name + "</a></div></td><td>Dir</td><td>0</td><td>"+curItem.getLastMod()+"</td></tr>";
					}
				}
                return fileshtml;
			}
			...
			function showContent (gsdirss, gsfiless) {
				...
				jQuery('div.file').contextMenu({
					...
					function(action, el, pos) {
					   jQuery(el).doGSAction({action: action, script: o.script, type: 'file', lg: o.language, oidname: o.oidname});
				},
				manageGsMenu);
				...
				jQuery('div.directory').contextMenu({
					...
					function(action, el, pos) {
					    jQuery(el).doGSAction({action: action, script: o.script, type: 'dir',lg: o.language, oidname: o.oidname});
				},
				manageGsMenu);

			}
			
			function showTree(c, t ,e) {
			...
			function showRoot(){
				showTree( jQuery('#gs_dir_list'), escape(o.root),escape(o.oidname));
		...
		doGSAction: function(o) {
				    ...
				    buttons: [{ 
					           ...
					           click: function() {
								   ...
								   sendAndRefresh(o, dataForSend, true, function(data) {
										...
										jQuery('#gsimageviewer_content').html('<img src="'+ gs_makeUrl(o.script, jQuery.param(dataForSend) +'oidname='+ o.oidname +'&time='+ new Date().getTime())+'" id="gs_imageviewer_image"/>');  
								   });
					       }
				     }]
				});
			...
			function sendAndRefresh(o, dataForSend, refresh, callback, type) {
				...
				jQuery.ajax({
	      		      type: 'POST',
					  url: o.script,
					  data: jQuery.param(dataForSend) + '&oidname='+ o.oidname +'&time='+ new Date().getTime(),
					  dataType: type,
					  contentType : 'application/x-www-form-urlencoded;charset=utf-8',
